...
def app():
    ...
    columns = ['file_name', 'publication_name', 'date', 'content']
    ...
    df['publication_name'] = df['file_name'].apply(lambda x: get_pub_name(get_ref(x)))
    df['date'] = df['file_name'].apply(lambda x: get_date(x))

    st.dataframe(df)
    
    #file_names = np.array([i.name for i in uploaded_files]) 
