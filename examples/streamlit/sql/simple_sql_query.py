...
from kiara_modules.playground.markus.streamlit import init_session, find_all_aliases_of_type, set_workflow_input, \
    onboard_file
...
fp = st.file_uploader(label="Onboard table data (.csv)", type=["csv"], accept_multiple_files=True)
if fp:
    onboard_file(kiara=kiara, st=st, uploaded_file=fp)

selected_alias = st.selectbox("Select dataset", find_all_aliases_of_type(kiara, value_type="table"))
...
with st.form(key="sql_query"):
    ...
    st.write("*Note*: the query must use the relation name '***data***' (e.g. 'select \\* from ***data***)")
